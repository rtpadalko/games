...
import { buildDeck, Category } from './data/words';
...
export type Phase =
  | 'setup' // настройка
  | 'intro' // «Команда X готова?»
  | 'round' // активный раунд
  | 'summary' // итог раунда: +1/-1 по словам
  | 'scoreboard' // табло между раундами
  | 'winner'; // победитель

export interface RoundWord {
  word: string;
  guessed: boolean; // ✓ угадано (можно переключать в summary)
}
...
interface AliasState {
  ...
  targetScore: number;
  categories: Category[];
  penalizeSkip: boolean; // -1 за пропуск
  // игровая партия
  phase: Phase;
  deck: string[]; // оставшиеся слова
  deckIndex: number;
  currentTeamIndex: number;
  roundWords: RoundWord[]; // что было в этом раунде
  // действия
  ...
  setPenalizeSkip: (v: boolean) => void;
  startGame: () => void;
  startRound: () => void;
  finishRound: (words: RoundWord[]) => void;
  toggleWordGuessed: (index: number) => void;
  applyRoundResult: () => void; // зачисляет очки и решает: → scoreboard или winner
  nextTeam: () => void;
  resetAll: () => void;
}
...
export const useAliasStore = create<AliasState>()(
  persist(
    (set, get) => ({
      ...
      addTeam: (name) =>
        set((s) => {
          if (s.teams.length >= 4) return s;
          const used = new Set(s.teams.map((t) => t.color));
          const color =
            TEAM_COLORS.find((c) => !used.has(c.id))?.id ?? 'rose';
          return {
            teams: [
              ...s.teams,
      ...
      toggleWordGuessed: (index) =>
        set((s) => ({
          roundWords: s.roundWords.map((w, i) =>
      ...
      applyRoundResult: () => {
        ...
        const newTeams = teams.map((t, i) =>
          i === currentTeamIndex ? { ...t, score: t.score + delta } : t
        );
